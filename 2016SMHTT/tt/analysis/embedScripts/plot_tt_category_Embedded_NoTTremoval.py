...
file=ROOT.TFile("final_nominal_tt.root","r")
#fileEmb=ROOT.TFile("outputs_nominal_Mass/Tot_Embed.root","r")
fileEmb=ROOT.TFile("final_nominal_tt_embedded.root","r")
...
#   VV=file.Get(categories[i]).Get("VV")#AM
   VV=file.Get(categories[i]).Get("VVJ")
   VV.Add(file.Get(categories[i]).Get("VVT"))
...
#   TT=file.Get(categories[i]).Get("TT")#AM
   TT=file.Get(categories[i]).Get("TTJ")
   TT.Add(file.Get(categories[i]).Get("TTT"))
...
#   errorBand=file.Get(categories[i]).Get("TT").Clone()#AM
   errorBand=file.Get(categories[i]).Get("TTJ").Clone()
   #errorBand.Add(ZZ)
   errorBand.Add(VV)
   errorBand.Add(DYJ)
   errorBand.Add(DYL)
...
#   legende.AddEntry(SMH,"SM Higgs(125)x5.0","l")
   ...
   legende.AddEntry(DYJ,"Z#rightarrowLL (jet)","f")
   legende.AddEntry(DYL,"Z#rightarrowLL (lepton)","f")
   ...
   categ.AddText('Inclusing TTT+VVT')
   categ.Draw()
...
#   h1.GetXaxis().SetTitle("Higgs p_{t} [GeV]")#M_{jj} [GeV]")#"M_{#tau#tau} [GeV]")
   ...
   c.Modified()
   c.SaveAs("htt_"+categories[i]+"_emb_ADDTT.pdf")
