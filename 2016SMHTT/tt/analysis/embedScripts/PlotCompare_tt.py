...
def MakeCompare(root1,catego,hist1,root2,hist2,dataset ,lumi,Xaxis,cat,includeContamination):
    ...
    if includeContamination:
        Histo1.Add(file1.Get(catego+'/VVT'))
...
#    leg.SetBorderSize(0)
    ...
    if includeContamination:
        leg.AddEntry(Histo1,'DY#rightarrowll (ZTT) + TTT + VVT','lp')
...
#    h5.Divide(h2)
    ...
    if includeContamination:
        h1.GetYaxis().SetTitle("DY+VVT+TTT / Embedded")
...
#    h5.Draw("same")
    ...
    if includeContamination:
        finalName='_Embed_New_tt_%s_%s_Normal_plus_TTT_VVT.pdf'%(cat,dataset)
...
plotInputEmbed=[
#        ['embed_B.root','2016B',5.93],
#        ['embed_C.root','2016C',2.65],
#        ['embed_D.root','2016D',4.35],
#        ['embed_E.root','2016E',4.12],
#        ['embed_F.root','2016F',3.19],
#        ['embed_G.root','2016G',7.72],
#        ['embed_H.root','2016H',8.86],
        ['final_nominal_tt_embedded.root','2016B-H',35.9],
...
for cont in (0,1):
    ...
    #            MakeCompare(input+'Tot_ZTT.root', cat+'/'+'ZTT' , input+plotInputEmbed[j][0], cat+'/'+FileNamesInfo[i][0],dataset ,lumi,Xaxis,cat)
                MakeCompare(input+'final_nominal_tt.root', cat,'/'+'ZTT' , input+plotInputEmbed[j][0], cat+'/'+FileNamesInfo[i][0],dataset ,lumi,Xaxis,cat,cont)




TFile *_file_B = TFile::Open("Skim_tt_B.root")
TTree * tt_B = (TTree *)_file_B->Get("tt_tree")
tt_B->Draw("Q2V2>>h","njets==2")
tt_B->Draw("Q2V2>>h","njets==2&&Q2V2>0")



TFile *_file_C = TFile::Open("Skim_tt_C.root")
TTree * tt_C = (TTree *)_file_C->Get("tt_tree")
tt_C->Draw("Q2V2>>h","njets==2")
tt_C->Draw("Q2V2>>h","njets==2&&Q2V2>0")



TFile *_file_D = TFile::Open("Skim_tt_D.root")
TTree * tt_D = (TTree *)_file_D->Get("tt_tree")
tt_D->Draw("Q2V2>>h","njets==2")
tt_D->Draw("Q2V2>>h","njets==2&&Q2V2>0")



TFile *_file_E = TFile::Open("Skim_tt_E.root")
TTree * tt_E = (TTree *)_file_E->Get("tt_tree")
tt_E->Draw("Q2V2>>h","njets==2")
tt_E->Draw("Q2V2>>h","njets==2&&Q2V2>0")



TFile *_file_F = TFile::Open("Skim_tt_F.root")
TTree * tt_F = (TTree *)_file_F->Get("tt_tree")
tt_F->Draw("Q2V2>>h","njets==2")
tt_F->Draw("Q2V2>>h","njets==2&&Q2V2>0")



TFile *_file_G = TFile::Open("Skim_tt_G.root")
TTree * tt_G = (TTree *)_file_G->Get("tt_tree")
tt_G->Draw("Q2V2>>h","njets==2")
tt_G->Draw("Q2V2>>h","njets==2&&Q2V2>0")



TFile *_file_H = TFile::Open("Skim_tt_H.root")
TTree * tt_H = (TTree *)_file_H->Get("tt_tree")
tt_H->Draw("Q2V2>>h","njets==2")
tt_H->Draw("Q2V2>>h","njets==2&&Q2V2>0")
