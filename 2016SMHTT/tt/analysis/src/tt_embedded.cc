...
#include "EmbedWeight.h" //AM
//#include "makeHisto.h"   //AM
...
int main(int argc, char** argv) {
    ...
    PyObject* compute_trg_eff_data = PyObject_GetAttrString(fitFunctions,"compute_Trg_Eff_Data");
    ...
    if (sample!="embedded" &&  w_lumi==0) std::cout << std::endl << "!!!!!!!!!!!!!!!!!!!!!!!! ATTENTION - can't find lumi weight. Check the sample. !!!!!!!!!!!!!!!!!!!!!!!!" << std::endl << std::endl;
    weight = w_lumi;
...
//    arbre->SetBranchAddress("GenWeight",&GenWeight);
    arbre->SetBranchAddress("amcatNLO_weight",&GenWeight);
...
//	bool tcomb35 =  passDoubleTauCmbIso35 && filterDoubleTauCmbIso35_1 && filterDoubleTauCmbIso35_2 && matchDoubleTauCmbIso35_1 && matchDoubleTauCmbIso35_2;
          ...
          bool t35     =  passDoubleTau35 ;
          bool tcomb35 =  passDoubleTauCmbIso35;

          
//AM remove the trigger //	if (  !t35 && !tcomb35 ) continue;
      }

        
        
        //AM
        if (sample=="embedded" && GenWeight > 1) continue;
	...
	if (gen_match_1==6) {
	  ...
	  sf_trg_FF = PyFloat_AsDouble(trgSF);
          ...
          cout << "sf_trg1 = "<<sf_trg1<< "   gen_match_1" << sf_trg_RR <<"\n";
	...
	if (gen_match_2==5)  {
	  sf_trg_RR = sf_trg_RR*PyFloat_AsDouble(trgSF);
        ...
        if (gen_match_2==6) {
          ...
          cout << "sf_trg2 = "<<sf_trg2<< "   gen_match_2" << sf_trg_RR <<"\n";
...
//&&&&&&&&&
        float trgEff1=1;
//        PyObject* trgEff = PyObject_CallFunction(compute_trg_eff_data,(char*)"[f,i]",0.0,0);
//            if (t1_decayMode==0) trgEff = PyObject_CallFunction(compute_trg_eff_data,(char*)"[f,i]",mytau1.Pt(),0);
//            else if (t1_decayMode==1) trgEff = PyObject_CallFunction(compute_trg_eff_data,(char*)"[f,i]",mytau1.Pt(),1);
//            else if (t1_decayMode==10) trgEff = PyObject_CallFunction(compute_trg_eff_data,(char*)"[f,i]",mytau1.Pt(),10);
//            trgEff1 = PyFloat_AsDouble(trgEff);

        
        
        
//        cout <<" trgEff1--->" << trgEff1<<  "    t1_decayMode="<<t1_decayMode << "  t2_decayMode="<<t2_decayMode <<"    sf_trg1="<<sf_trg1 << "  sf_trg2="<<sf_trg2 << "   gen_match_1="<<gen_match_1<<  "  tau pt1== "<< mytau1.Pt()<<"  tau pt2== "<< mytau2.Pt()<< "\n"  ;
	...
	if (sample=="data_obs") {aweight=1.0; weight2=1.0;}
          ...
          //        cout <<"Stitching_Weight = "<<Stitching_Weight<<"\n";
          
          
//          vector<double> info=EmdWeight_Tau(RooWorkspace *wEmbed, float pt, float TauDM  wEmbed,mymu.Pt(),mymu.Eta(),iso_1);
//          
...
//          double EmbedWeight=muon_id_scalefactor*muon_iso_scalefactor*muon_trg_efficiency;
          ...
          //        double EmbedWeight=muon_id_scalefactor*muon_iso_scalefactor;
          ...
          if (sample=="embedded") {
              ...
              cout <<EmbedWeight <<  "  "<< GenWeight <<  "  "<< Stitching_Weight <<  "  "<< WEIGHT_sel_trg_ratio<<"\n";
              
              Total_Embed_Weight=EmbedWeight * GenWeight * Stitching_Weight * WEIGHT_sel_trg_ratio;
              
              //            cout <<Total_Embed_Weight<<"  "<<EmbedWeight <<"  "<< GenWeight <<"  "<< Stitching_Weight <<"  "<< WEIGHT_sel_trg_ratio<<"  \n";
          }
          //        if (EmbedWeight < 0.5)  cout<<muon_id_scalefactor<<"  "<<muon_iso_scalefactor<<"  "<<muon_trg_efficiency<<"  \n";
	...
	if (selection){
	  ...
	  if (is_0jet && signalRegion && charge1*charge2<0){
	    h0_OS[k]->Fill(var,Total_Embed_Weight);
	    if (tes==0)
	      h_0jet->Fill(var,Total_Embed_Weight);
	  }
	  if (is_boosted && signalRegion && charge1*charge2<0){
	    h1_OS[k]->Fill(var1_1,var2,Total_Embed_Weight);
	    if (tes==0){
	  ...
	  if (is_VBF && signalRegion && charge1*charge2<0) {
	    h2_OS[k]->Fill(var1_2,var2,Total_Embed_Weight);
	    if (tes==0){
	  ...
	  if (is_VH && signalRegion && charge1*charge2<0)
	    h3_OS[k]->Fill(var1_2,var2,Total_Embed_Weight);
	  if (signalRegion && charge1*charge2<0)
	    h_OS[k]->Fill(var,Total_Embed_Weight);

	  // tmp histograms for R&D
	  if (is_0jet && signalRegion && charge1*charge2<0) h0_rndOS[k]->Fill(varRND,Total_Embed_Weight);
	  if (is_1jet && signalRegion && charge1*charge2<0) h1_rndOS[k]->Fill(varRND,Total_Embed_Weight);
	  if (is_2jets && signalRegion && charge1*charge2<0) h2_rndOS[k]->Fill(varRND,Total_Embed_Weight);
	  
	  
	  // ################### signalRegion && SS ####################
	  if (is_0jet && signalRegion && charge1*charge2>0)
	    h0_SS[k]->Fill(var,Total_Embed_Weight);
	  if (is_boosted && signalRegion && charge1*charge2>0)
	    h1_SS[k]->Fill(var1_1,var2,Total_Embed_Weight);
	  if (is_VBF && signalRegion && charge1*charge2>0) {
	    h2_SS[k]->Fill(var1_2,var2,Total_Embed_Weight);
	  }
	  if (is_VH && signalRegion && charge1*charge2>0)
	    h3_SS[k]->Fill(var1_2,var2,Total_Embed_Weight); 
	  if (signalRegion && charge1*charge2>0)
	    h_SS[k]->Fill(var,Total_Embed_Weight);

	  // tmp histograms for R&D
	  if (is_0jet && signalRegion && charge1*charge2>0) h0_rndSS[k]->Fill(varRND,Total_Embed_Weight);
	  if (is_1jet && signalRegion && charge1*charge2>0) h1_rndSS[k]->Fill(varRND,Total_Embed_Weight);
	  if (is_2jets && signalRegion && charge1*charge2>0) h2_rndSS[k]->Fill(varRND,Total_Embed_Weight);

	  // ################### ai-Region && OS ####################
	  if (is_0jet && charge1*charge2<0 && aiRegion)
	    h0_AIOS[k]->Fill(var,Total_Embed_Weight);
	  if (is_boosted && charge1*charge2<0 && aiRegion)
	    h1_AIOS[k]->Fill(var1_1,var2,Total_Embed_Weight);
	  if (is_VBF && charge1*charge2<0 && aiRegion) {
	    h2_AIOS[k]->Fill(var1_2,var2,Total_Embed_Weight);
	  }
	  if (is_VH && charge1*charge2<0 && aiRegion)
	    h3_AIOS[k]->Fill(var1_2,var2,Total_Embed_Weight);
	  if (charge1*charge2<0 && aiRegion)
	    h_AIOS[k]->Fill(var,Total_Embed_Weight);

	  // tmp histograms for R&D
	  if (is_0jet && aiRegion && charge1*charge2<0) h0_rndAIOS[k]->Fill(varRND,Total_Embed_Weight);
	  if (is_1jet && aiRegion && charge1*charge2<0) h1_rndAIOS[k]->Fill(varRND,Total_Embed_Weight);
	  if (is_2jets && aiRegion && charge1*charge2<0) h2_rndAIOS[k]->Fill(varRND,Total_Embed_Weight);

	  
	  // ################### ai-Region && SS ####################
	  if (is_0jet && charge1*charge2>0 && aiRegion)
	    h0_AISS[k]->Fill(var,Total_Embed_Weight);
	  if (is_boosted && charge1*charge2>0 && aiRegion)
	    h1_AISS[k]->Fill(var1_1,var2,Total_Embed_Weight);
	  if (is_VBF && charge1*charge2>0 && aiRegion) {
	    h2_AISS[k]->Fill(var1_2,var2,Total_Embed_Weight);
	  }
	  if (is_VH && charge1*charge2>0 && aiRegion)
	    h3_AISS[k]->Fill(var1_2,var2,Total_Embed_Weight);
	  if (charge1*charge2>0 && aiRegion)
	    h_AISS[k]->Fill(var,Total_Embed_Weight);	  

	  // tmp histograms for R&D
	  if (is_0jet && aiRegion && charge1*charge2>0) h0_rndAISS[k]->Fill(varRND,Total_Embed_Weight);
	  if (is_1jet && aiRegion && charge1*charge2>0) h1_rndAISS[k]->Fill(varRND,Total_Embed_Weight);
	  if (is_2jets && aiRegion && charge1*charge2>0) h2_rndAISS[k]->Fill(varRND,Total_Embed_Weight);
