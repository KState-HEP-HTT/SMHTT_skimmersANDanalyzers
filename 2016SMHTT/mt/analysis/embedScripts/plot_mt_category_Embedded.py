...
file=ROOT.TFile("final_nominal_mt.root","r")
#fileEmb=ROOT.TFile("outputs_nominal_Mass/Tot_Embed.root","r")
fileEmb=ROOT.TFile("final_nominal_mt_embedded.root","r")

file_TTT_excl=ROOT.TFile("TTT_excluding_digentau.root","r")
...
#categories=["tt_0jet","tt_boosted","tt_vbf","tt_inclusive"]
categories=["mt_0jet","mt_boosted","mt_vbf"]
#categories=["tt_inclusive"]



for i in range (0,len(categories)):
   Data=file.Get(categories[i]).Get("data_obs")
#   VV=file.Get(categories[i]).Get("VV")#AM
   VV=file.Get(categories[i]).Get("VV")
   DYTT=fileEmb.Get(categories[i]).Get("embedded")
   DYL=file.Get(categories[i]).Get("ZL")
   DYJ=file.Get(categories[i]).Get("ZJ")
#   TT=file.Get(categories[i]).Get("TT")#AM
   TT=file_TTT_excl.Get(categories[i]).Get("TTT_excluding_digentau")#AM
   TT.Add(file.Get(categories[i]).Get("TTJ"))
   QCD=file.Get(categories[i]).Get("QCD")
#   SMH=file.Get(categories[i]).Get("ggH125")#AM FIXME
...
#   SMH.Scale(5)
   ...
   DYTT.SetFillColor(ROOT.TColor.GetColor(248,126,0))
...
#   errorBand=file.Get(categories[i]).Get("TT").Clone()#AM
   errorBand=file.Get(categories[i]).Get("VV").Clone()
   #errorBand.Add(ZZ)
   errorBand.Add(TT)
   errorBand.Add(DYJ)
...
#   legende.AddEntry(SMH,"SM Higgs(125)x5.0","l")
   ...
   legende.AddEntry(DYTT,"Embedded","f")
   legende.AddEntry(DYJ,"Z#rightarrow#mu#mu (jet)","f")
   legende.AddEntry(DYL,"Z#rightarrow#mu#mu (lepton)","f")
   ...
   categ  = ROOT.TPaveText(0.21, 0.55+0.013, 0.43, 0.60+0.155, "NDC")
...
#   categ.SetTextFont (   41 )
   ...
   categ.AddText('t#bar{t}: excluding genuine #tau#tau')
   categ.Draw()
   ...
   h1.Divide(hwoE)
   h3.Divide(hwoE)
   h1.GetXaxis().SetTitle("M_{#tau#tau} [GeV]")#M_{jj} [GeV]")#"M_{#tau#tau} [GeV]")
#   h1.GetXaxis().SetTitle("Higgs p_{t} [GeV]")#M_{jj} [GeV]")#"M_{#tau#tau} [GeV]")
   ...
   c.Modified()
   c.SaveAs("htt_"+categories[i]+"_emb_new.pdf")
