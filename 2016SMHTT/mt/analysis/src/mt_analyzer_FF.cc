...
int main(int argc, char** argv) {
    ...
    */
    ...
    for (int k=0; k<nbhist; ++k){
      ...
      h2_WOS.push_back(new TH1F (HNS2WOS.str().c_str(),"InvMa",binnum_mjj,bins_mjj)); h2_WOS[k]->Sumw2();
        ...
        h0tau_OS.push_back(new TH1F (HNSh0tau_OS.str().c_str(),"InvMa",binnum_taupt,bins_taupt)); h0tau_OS[k]->Sumw2();
        h0ell_OS.push_back(new TH1F (HNSh0ell_OS.str().c_str(),"InvMa",binnum_pth,bins_pth)); h0ell_OS[k]->Sumw2();
        h0_AI.push_back(new TH1F (HNSh0_AI.str().c_str(),"InvMa",binnum_mjj,bins_mjj)); h0_AI[k]->Sumw2();
        ...
        h1tau_OS.push_back(new TH1F (HNSh1tau_OS.str().c_str(),"InvMa",binnum_taupt,bins_taupt)); h1tau_OS[k]->Sumw2();
        h1ell_OS.push_back(new TH1F (HNSh1ell_OS.str().c_str(),"InvMa",binnum_pth,bins_pth)); h1ell_OS[k]->Sumw2();
        h1_AI.push_back(new TH1F (HNSh1_AI.str().c_str(),"InvMa",binnum_mjj,bins_mjj)); h1_AI[k]->Sumw2();
        ...
        h2tau_OS.push_back(new TH1F (HNSh2tau_OS.str().c_str(),"InvMa",binnum_taupt,bins_taupt)); h2tau_OS[k]->Sumw2();
        h2ell_OS.push_back(new TH1F (HNSh2ell_OS.str().c_str(),"InvMa",binnum_pth,bins_pth)); h2ell_OS[k]->Sumw2();
        h2_AI.push_back(new TH1F (HNSh2_AI.str().c_str(),"InvMa",binnum_mjj,bins_mjj)); h2_AI[k]->Sumw2();
  ...
  TString postfixFF[31]={"","_norm_ff_qcd_mt_systUp","_norm_ff_qcd_mt_systDown","_norm_ff_qcd_dm0_njet0_mt_statUp","_norm_ff_qcd_dm0_njet0_mt_statDown","_norm_ff_qcd_dm0_njet1_mt_statUp","_norm_ff_qcd_dm0_njet1_mt_statDown","_norm_ff_qcd_dm1_njet0_mt_statUp","_norm_ff_qcd_dm1_njet0_mt_statDown","_norm_ff_qcd_dm1_njet1_mt_statUp","_norm_ff_qcd_dm1_njet1_mt_statDown","_norm_ff_w_mt_systUp","_norm_ff_w_mt_systDown","_norm_ff_w_dm0_njet0_mt_statUp","_norm_ff_w_dm0_njet0_mt_statDown","_norm_ff_w_dm0_njet1_mt_statUp","_norm_ff_w_dm0_njet1_mt_statDown","_norm_ff_w_dm1_njet0_mt_statUp","_norm_ff_w_dm1_njet0_mt_statDown","_norm_ff_w_dm1_njet1_mt_statUp","_norm_ff_w_dm1_njet1_mt_statDown","_norm_ff_tt_mt_systUp","_norm_ff_tt_mt_systDown","_norm_ff_tt_dm0_njet0_mt_statUp","_norm_ff_tt_dm0_njet0_mt_statDown","_norm_ff_tt_dm0_njet1_mt_statUp","_norm_ff_tt_dm0_njet1_mt_statDown","_norm_ff_tt_dm1_njet0_mt_statUp","_norm_ff_tt_dm1_njet0_mt_statDown","_norm_ff_tt_dm1_njet1_mt_statUp","_norm_ff_tt_dm1_njet1_mt_statDown"};
   ...
   // AM  bool isSingleLep = (passIsoMu22 && matchIsoMu22_1 && filterIsoMu22_1) or (passIsoTkMu22 && matchIsoTkMu22_1 && filterIsoTkMu22_1) or (passIsoMu22eta2p1 && matchIsoMu22eta2p1_1 && filterIsoMu22eta2p1_1) or (passIsoTkMu22eta2p1 && matchIsoTkMu22eta2p1_1 && filterIsoTkMu22eta2p1_1);
...
//std::cout<<"  gen_match_1 & gen_match_1 = "<< gen_match_1<<"  "<<gen_match_2<<"\n";	
...
//############################################################################################################
        ...
        //*********************  fake factors weight *********************
        double FF_weight=ff_0jet->value(inputs);

        float var=m_sv;
        //************************* Fill histograms Fake Factor 0 jet **********************
        
        if (numberJets==0 && mt<50 && mytau.Pt()>30){
            
            if ((sample=="data_obs" or gen_match_2==5) && signalRegion && q_1*q_2<0) // had taus
                h0tau_OS[k]->Fill(var,aweight*weight2);
            
            if ((sample!="data_obs" && gen_match_2<5) && signalRegion && q_1*q_2<0) // light leptons
                h0ell_OS[k]->Fill(var,aweight*weight2);

            if (antiisoRegion && q_1*q_2<0 && (sample=="data_obs" or gen_match_2<=5)){ //anti-isolated data, and MC without jets to subtract later
//                for (int k=l;k<l+nbhistFF;++k){ // To get also the systematics
//                    double FF=ff_0jet->value(inputs,FFsys[0]);
//                    double FF=ff_0jet->value(inputs);
                    h0_AI[k]->Fill(var,aweight*FF_weight*weight2);
//                }
            }
        }
        ...
        if (is_boosted && mt<50 && mytau.Pt()>30){
            
            if ((sample=="data_obs" or gen_match_2==5) && signalRegion && q_1*q_2<0) // had taus
                h1tau_OS[k]->Fill(var,aweight*weight2);
            
            if ((sample!="data_obs" && gen_match_2<5) && signalRegion && q_1*q_2<0) // light leptons
                h1ell_OS[k]->Fill(var,aweight*weight2);
            
            if (antiisoRegion && q_1*q_2<0 && (sample=="data_obs" or gen_match_2<=5)){ //anti-isolated data, and MC without jets to subtract later
                //                for (int k=l;k<l+nbhistFF;++k){ // To get also the systematics
                //                    double FF=ff_0jet->value(inputs,FFsys[0]);
                //                    double FF=ff_0jet->value(inputs);
                h1_AI[k]->Fill(var,aweight*FF_weight*weight2);
                //                }
            }
        }
        ...
        if (is_VBF && mt<50 && mytau.Pt()>30){
            
            if ((sample=="data_obs" or gen_match_2==5) && signalRegion && q_1*q_2<0) // had taus
                h2tau_OS[k]->Fill(var,aweight*weight2);
            
            if ((sample!="data_obs" && gen_match_2<5) && signalRegion && q_1*q_2<0) // light leptons
                h2ell_OS[k]->Fill(var,aweight*weight2);
            
            if (antiisoRegion && q_1*q_2<0 && (sample=="data_obs" or gen_match_2<=5)){ //anti-isolated data, and MC without jets to subtract later
                //                for (int k=l;k<l+nbhistFF;++k){ // To get also the systematics
                //                    double FF=ff_0jet->value(inputs,FFsys[0]);
                //                    double FF=ff_0jet->value(inputs);
                h2_AI[k]->Fill(var,aweight*FF_weight*weight2);
                //                }
            }
        }
...
//############################################################################################################
	  ...
	  if (qcdCR && q_1*q_2<0 && mt<50){
	    if (is_0jet)
	      h0_CR_QCD[k]->Fill(varX,aweight*weight2*ratioanti);
	    if (is_boosted)
	      h1_CR_QCD[k]->Fill(varX,aweight*weight2*ratioanti);
	    if (is_VBF)
	      h2_CR_QCD[k]->Fill(varX,aweight*weight2*ratioanti);
          if (is_Inclusive)
          h3_CR_QCD[k]->Fill(varX,aweight*weight2*ratioanti);
	  }
	  if (signalRegion && q_1*q_2<0){
	    ...
	    if (is_VBF && is_bveto && var2<400)
          ...
          if (is_Inclusive && is_bveto && var2<400)
              h3_CR_W[k]->Fill(mt,weight2*aweight*weight_btag);
	  ...
	  if (qcdCR && q_1*q_2>0 && mt<50){
	    if (is_0jet)
	      h0_CRSS_QCD[k]->Fill(varX,weight2*aweight*ratioanti);
	    if (is_boosted)
	      h1_CRSS_QCD[k]->Fill(varX,weight2*aweight*ratioanti);
	    if (is_VBF)
	      h2_CRSS_QCD[k]->Fill(varX,weight2*aweight*ratioanti);
          if (is_Inclusive)
              h3_CRSS_QCD[k]->Fill(varX,weight2*aweight*ratioanti);
	  ...
	  if (signalRegion && q_1*q_2>0){
	    ...
	    if (is_VBF && is_bveto && var2<400)
          ...
          if (is_Inclusive && is_bveto && var2<400)
              h3_CRSS_W[k]->Fill(mt,weight2*aweight*weight_btag);
        ...
        if (is_Inclusive && mt<50){
            ...
            if (signalRegion && q_1*q_2<0){
                if (l2_decayMode==0) h2D_inclusive->Fill(varX,weight2*aweight);
                if (l2_decayMode==1) h2D_inclusive->Fill(varX,weight2*aweight);
                if (l2_decayMode==10) h2D_inclusive->Fill(varX,weight2*aweight);
                h3_OS[k]->Fill(varX,weight2*aweight);
            }
            if (signalRegion && q_1*q_2>0)
                h3_SS[k]->Fill(varX,weight2*aweight);
            if (wRegion && q_1*q_2<0)
                h3_WOS[k]->Fill(varX,weight2*aweight);
            if (wRegion && q_1*q_2>0)
                h3_WSS[k]->Fill(varX,weight2*aweight);
            if (qcdRegion && q_1*q_2>0)
                h3_QCD[k]->Fill(varX,weight2*aweight);
        }
	  ...
	  if (is_0jet && mt<50){
	    ...
	    if (signalRegion && q_1*q_2<0){
	      pteta_0jet->Fill(varX,weight2*aweight);
	      ...
	      h0_OS[k]->Fill(varX,weight2*aweight);
	      if (l2_decayMode==0) h2D_0jet->Fill(varX,weight2*aweight);
	      if (l2_decayMode==1) h2D_0jet->Fill(varX,weight2*aweight);
	      if (l2_decayMode==10) h2D_0jet->Fill(varX,weight2*aweight);
	    }
	    if (signalRegion && q_1*q_2>0)
	      h0_SS[k]->Fill(varX,weight2*aweight);
	    if (wRegion && q_1*q_2<0)
	      h0_WOS[k]->Fill(varX,weight2*aweight);
	    if (wRegion && q_1*q_2>0)
	      h0_WSS[k]->Fill(varX,weight2*aweight);
	    if (qcdRegion && q_1*q_2>0)
	      h0_QCD[k]->Fill(varX,weight2*aweight);
	  }
	  ////////////////////////////////////////////////////////////////////////
	  ...
	  if (is_boosted && mt<50){
	    ...
	    if (genpT>150 and tes==-22) weight2=weight2*(2-(1+0.01*(0.114*genpT -17.14)));
	    if (signalRegion && q_1*q_2<0){
	      h1_OS[k]->Fill(varX,weight2*aweight);
	      h2D_boosted->Fill(varX,weight2*aweight);
	      if (var1_1>150) {
		...
		mTESDown_boosted->Fill((m_sv_DOWN-m_sv)/m_sv_DOWN,weight2*aweight);
	      ...
	      if (var1_1<100) pteta_boosted_1->Fill(varX,weight2*aweight);
	      else if (var1_1<150) pteta_boosted_2->Fill(varX,weight2*aweight);
	      else if (var1_1<200) pteta_boosted_3->Fill(varX,weight2*aweight);
	      else if (var1_1<250) pteta_boosted_4->Fill(varX,weight2*aweight);
	      else if (var1_1<300) pteta_boosted_5->Fill(varX,weight2*aweight);
	      else pteta_boosted_6->Fill(varX,weight2*aweight);
	    }
	    if (signalRegion && q_1*q_2>0)
	      h1_SS[k]->Fill(varX,weight2*aweight);
	    if (wRegion && q_1*q_2<0)
	      h1_WOS[k]->Fill(varX,weight2*aweight);
	    if (wRegion && q_1*q_2>0)
	      h1_WSS[k]->Fill(varX,weight2*aweight);
	    if (qcdRegion && q_1*q_2>0)
	      h1_QCD[k]->Fill(varX,weight2*aweight);
	  }
	  ////////////////////////////////////////////////////////////////////////
	  ...
	  if (is_VBF && mt<50){
	    ...
	    if (signalRegion && q_1*q_2<0){
	      ...
	      h2_OS[k]->Fill(varX,weight2*aweight);
	      //std::cout << " --------------------- here --------------------- " << std::endl;
	      h2D_vbf->Fill(varX,weight2*aweight);
	      if (var1_2<700) pteta_vbf_1->Fill(varX,weight2*aweight);
	      else if (var1_2<1100) pteta_vbf_2->Fill(varX,weight2*aweight);
	      else if (var1_2<1500) pteta_vbf_3->Fill(varX,weight2*aweight);
	      else pteta_vbf_4->Fill(varX,weight2*aweight);
	    ...
	    if (signalRegion && q_1*q_2>0)
	      h2_SS[k]->Fill(varX,weight2*aweight);
	    if (wRegion && q_1*q_2<0)
	      h2_WOS[k]->Fill(varX,weight2*aweight);
	    if (wRegion && q_1*q_2>0)
	      h2_WSS[k]->Fill(varX,weight2*aweight);
	    if (qcdRegion && q_1*q_2>0)
	      h2_QCD[k]->Fill(varX,weight2*aweight);
	  }
	}
    ...
    //FF
    TDirectory *OS0jet_FF =fout->mkdir("mt_0jet_FF");
    TDirectory *AI0jet_FF =fout->mkdir("AI0jet_FF");
    
    TDirectory *OSboosted_FF =fout->mkdir("mt_boosted_FF");
    TDirectory *AIboosted_FF =fout->mkdir("AIboosted_FF");
    
    TDirectory *OSvbf_FF =fout->mkdir("mt_vbf_FF");
