...
int main(int argc, char** argv) { 
    ...
    TTree *Run_Tree = new TTree("mt_tree", "mt_tree");
    ...
    // Trigger flags
    Run_Tree->Branch("trg_singlemuon", &trg_singlemuon, "trg_singlemuon/F"); // FIXME
    // Leg 1
    ...
    Run_Tree->Branch("id_m_loose_1",&id_m_loose_1,"id_m_loose_1/F");
    ...
    pair<float, float> muCandidate, tauCandidate;

    // Starts loop over events 
    for (int iEntry = 0; iEntry < tree->GetEntries() ; iEntry++)
    {
      // For tau pair forming algorithm
      pair<float, float> currentMuCandidate, currentTauCandidate;
      ...
      */
      ...
      // Muon
      if (mu.Pt() < 21 || fabs(mu.Eta()) > 2.1 ) continue;
	...
	continue;
      ...
      // Pair
      if (mu.DeltaR(tau) < 0.5) continue;

      if (tree->vbfJetVeto30 > 10)  cout << endl <<  "tree->vbfJetVeto30 > 10 : " << tree->jetVeto20 << ", " << tree->vbfJetVeto20 << ", " << tree->jetVeto30 << ", " << tree->vbfJetVeto30 << endl;
	  ...
	  if (evt_now==evt_before){
	    if (tree->mPt + tree->tPt >lt_before+0.0001){
	      bestEntry=iEntry;
	      lt_before=tree->mPt + tree->tPt;
	    }
	  }
	  ...
	  if ( evt_now!=evt_before) { // new event, save the tau candidates
	    ...
	    bestEntry=iEntry;
	    muCandidate  = make_pair(tree->mPt,  tree->mRelPFIsoDBDefaultR04);
	    ...
	    if ( fabs(currentMuCandidate.second - muCandidate.second)  <  0.0001 ) {
    ...
    if (bestEntry>-1) fillTree(Run_Tree,tree,bestEntry,isMC);
